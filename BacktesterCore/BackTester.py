...
class BacktestCandle:
    indicatorsNames = ["OHLC/4", "RSI(14)", "RSI(42)", "BB_HIGHER", "BB_LOWER", "MACD(12, 26, 9)", "AROON_OSC",
                       "OTHERS"]
    SMAs_periods = [8, 20, 50, 100, 200, 400]
    EMAs_periods = [8, 20, 50, 100, 200, 400]

    def __init__(self, timestamp, open, high, low, close, **kwargs):
        if open < close:
            self.bullish = True
        else:
            self.bullish = False
    ...
    def getPercentageChange(self):
        try:
            if not self.bullish:
                ...
                return ((self.open / self.close) - 1) * -100  #incremento
        except ZeroDivisionError:
            return float('inf')

    def getHeikenAshi(self, previousCandle):
        return BacktestHeikenCandle(previousCandle.timestamp, (previousCandle.open + previousCandle.close) / 2,
                                    max(self.high, self.open, self.close),
                                    min(self.low, self.open, self.close),
                                    (self.open + self.high + self.low + self.close) / 4, isHeiken=True)
    ...
    def __str__(self):
        if self.isHeiken:
            ...
            str = f"OPEN: {self.open} $ ---> CLOSE: {self.close} $ = {numpy.round(self.getPercentageChange(), 4)} % ({timestampToDate(self.timestamp)})\nIndicators: {self.indicatorsNames}\nEMAS: {self.EMAs}\nSMAS: {self.SMAs}\n"
        return str
...
class BacktestTrade:
    ...
    def getAgeInCandles(self, currentTimestamp, timeframe):
    ...
    def getPNL(self, currentPrice):
        if self.isBuy:
            ...
            return ((self.entryPrice / currentPrice) - 1) * 100

    def closeTrade(self, closingPrice, timestampExit):
    ...
    def __str__(self):
        ...
        if self.timestampEntry < self.timestampExit and self.exitPrice > 0:
            ...
            return f"{side} trade - ({timestampToDate(self.timestampEntry)}) [Opened: {order1} at {self.entryPrice} $ ---> Trade still open]\n"
...
class BacktestLeveragedTrade(BacktestTrade):
    ...
    def __init__(self, timestampEntry, isBuy, entryPrice, qty, leverage):
        ...
        if leverage > 1:
            ...
            raise AttributeError("Levarege must be greater than 1")
    ...
    def loadYearlyBacktest(self, timeframe, market, startingYear, extraYears):
        if startingYear > 2009:
            if extraYears >= 1:
                ...
                self.dataset.loadCandlesFromFile(candlesDataFilepath(timeframe, market, f"{startingYear}"))
        else:
            raise AttributeError(f"{startingYear} is too small!")
...
class BacktestTrendStrategy:
    ...
    def preloadTrendStrategy(self, datasetCandles):
        ...
        for score in range(self.MIN_ACT_SCORE, self.MAX_ACT_SCORE + 1):
            ...
            for candle in datasetCandles:
                ...
                def resetTrendScore(resetIter):
                    ...
                    if candle.indicators.get("RSI(14)") - candle.indicators.get(
                            "RSI(42)") >= 20 and iter - resetIter >= self.NORMAL_RESET_FACTOR:
                        self.bull = int(score / 2)
                    elif candle.indicators.get("RSI(14)") - candle.indicators.get(
                            "RSI(42)") <= -20 and iter - resetIter >= self.NORMAL_RESET_FACTOR:
    ...
    def preloadHeikenStrategy(self, heikenCandles):
        pass
...
class BacktestDataset:
    ...
    def displayRandomCandles(self):
        def displayFixedInterval(startingFrom):
            print("Normal:\n", self.candles[startingFrom], self.candles[startingFrom + 1],
                  self.candles[startingFrom + 2])
            print("HeikenAshi:\n", self.heikenAshiCandles[startingFrom], self.heikenAshiCandles[startingFrom + 1],
                  self.heikenAshiCandles[startingFrom + 2])
    ...
    def getDatasetStats(self, quantilesValue):
        ...
        def getSTDforPriceVariation(absolute):
            ...
            for candle in self.candles:
                if absolute:
                    ...
                    percentageChanges.append(candle.getPercentageChange())
        ...
        if quantilesValue > 1:
            ...
            statsStr = {
                "content": f"Standard Deviation of price variation --> Absolute values: {std_8rounding(getSTDforPriceVariation(True))} %, "
            ...
            return "Invalid quantiles value!"
    ...
    def getLoadedDates(self, heiken):
        dates = []
        if heiken:
            candleSrc = self.heikenAshiCandles
        else:
            candleSrc = self.candles
    ...
    def getLoadedSMA(self, period):
        if period in BacktestCandle.SMAs_periods:
            ...
            raise AttributeError(f"SMA period {period} is unavailable")
    ...
    def loadCandlesFromFile(self, filepath):
            ...
            for market in self.markets:
                if filepath.find(market) != -1:
                    if self.market is None:
                        ...
                        raise AttributeError("Check filepath market!")
        ...
        def loadHeikenAshi():
            if previouslyLoaded != 0:
                ...
                rng = range(previouslyLoaded, len(self.candles) - 1)
            for x in rng:
        ...
        try:
            ...
            while len(params) > 0:
                ...
                if len(params) >= 5:
                    candle = BacktestCandle(int(params[0]), float(params[1]), float(params[2]), float(params[3]),
                                            float(params[4]), isHeiken=False)
    ...
    def loadAdditionalCandles(self, filepath):
        pass
    ...
    def __str__(self):
        if len(self.candles) != 0:
            ...
            return "No loaded candles in the dataset"
...
class BackTestRunner:
        ...
        def validPreset():
            try:
                return kwargs.get("initialUSD") >= 10 and 0 <= kwargs.get("startingAllocation") <= 1 and kwargs.get(
                    "tpValue") > 0 and (kwargs.get("initialUSD") * kwargs.get("startingAllocation")) > kwargs.get(
                    "tradesSizeUSD")
            except Exception:
    ...
    def runnerSetup(self, **kwargs):
        self.bot = BacktestBot(self.dataset, kwargs.get("initialUSD"), kwargs.get("tradesSizeUSD"),
                               kwargs.get("startingAllocation"), kwargs.get("tpValue"),
                               kwargs.get("plottable"), kwargs.get("adaptiveTradesSize"))

    def runSimpleStrategyBacktest(self, **kwargs):
        self.bot.applySimpleTrendScoreStrategy(kwargs.get("activationScore"), kwargs.get("maxOpen"),
                                               kwargs.get("closingAge"))
...
class BacktestBot:
    ...
    def __init__(self, dataset=BacktestDataset, initialUSD=int, tradesSizeUSD=int, startingAllocation=float,
        ...
        try:
            def validPreset():
                return initialUSD >= 10 and 0 <= startingAllocation <= 1 and tpValue > 0 and (
                            initialUSD * startingAllocation) > tradesSizeUSD
            ...
            if validPreset():
                ...
                if self.plottable:
                    ...
                    self.buyNholdTrade = BacktestTrade(dataset.candles[0].timestamp, True, dataset.candles[0].close,
                                                       self.initialCOIN)
    ...
    def resetBot(self):
        ...
        if self.plottable:
            self.plotSrc.clear()
            self.buyNholdTrade = BacktestTrade(self.dataset.candles[0].timestamp, True, self.dataset.candles[0].close,
                                               self.initialCOIN)
        if self.adaptiveTrades:
    ...
    def checkNegativeBalance(self, originTrade):
        if self.positionUSD < 0 or self.positionCOIN < 0: raise RuntimeError(
            f"{self.positionUSD} and {self.positionCOIN} resulted negative after this trade:\n{originTrade.__str__()}")
    ...
    def closeRemainingTrades(self):
        for trade in self.trades:
            if not trade.isClosed():
                if trade.isBuy and self.canCloseLong(trade):
                    trade.closeTrade(self.dataset.candles[len(self.dataset.candles) - 1].close,
    ...
    def applySimpleTrendScoreStrategy(self, activationScore, maxOpen, closingAge):
        if len(self.trades) == 0:
            ...
            for candle in self.dataset.candles:
                ...
                def updatePlottableData():
                    if self.plottable:
                        ...
                        bNhUSD.append(std_2rounding(
                            (1 + (self.buyNholdTrade.getPNL(currentPrice) / 100)) * self.buyNholdTrade.qtyUSD))
                        bNhPNLS.append(std_4rounding(self.buyNholdTrade.getPNL(currentPrice)))
                        totalUSD.append(std_2rounding(currentTotal)), pnls.append(
                            std_4rounding(((currentTotal / self.initialUSD) - 1) * 100))
                        efficencies.append(std_4rounding((((self.positionUSD + (
                                    self.positionCOIN * self.dataset.candles[0].close)) / self.initialUSD) - 1) * 100))
                        self.plotSrc.update({"buyNholdUSD": bNhUSD, "buyNholdPNL": bNhPNLS, "runningBotUSD": totalUSD,
                                             "runningPNL": pnls, "runningEff": efficencies})
                ...
                def tradeOpenerByBias():
                    if self.currentTradesOpen() <= maxOpen:
                        ...
                        return self.closeBestTradeUnderTP(currentPrice, currentTime)
            ...
            raise RuntimeError("This strategy has already been applied to the dataset!")
...
class BacktestLeveragedBot(BacktestBot):
    ...
    def __init__(self, dataset=BacktestDataset, initialUSD=int, tradesSizeUSD=int, startingAllocation=float,
        ...
        super().__init__(dataset, initialUSD, tradesSizeUSD, startingAllocation, tpValue, plotPerformance,
                         adaptiveTradesSize)
...
class BacktestStrategyOptimizer:
    ...
    def __init__(self, loadedDataset=BackTestLoader):
        ...
        self.strategyParams: List[tuple] = []
        self.bestPNL = None
        self.bestEfficiency = None
        self.winningBots = 0  # calculated by efficiency
        self.losingBots = 0  # calculated by efficiency
        self.WLratio = 0  # calculated by efficiency
    ...
    def validOptimization(self):
        return len(self.bots) == len(self.botsPNLs) == len(self.botsEfficencies) == len(self.strategyParams)
    ...
    def setBestBotByEfficency(self):
        for bot in self.bots:
            if bot.find(f"Starting price based PNL/Efficency: {std_4rounding(max(self.botsEfficencies))} %") != -1:
                self.bestEfficiency = bot
                break

    def randomizeSimpleStrategyParams(self, maxTP, maxScore, maxMaxOpen, maxClosingAge):
        # very heavy method, range step between input parameters is 1
        if self.loadedDataset.ready:
            ...
            if validRandomizationLimits():
                runner = BackTestRunner(self.loadedDataset.dataset, self.loadedDataset.dataset.timeframe,
                                        self.loadedDataset.dataset.market, initialUSD=DEFAULT_INITIAL,
                                        tradesSizeUSD=DEFAULT_TRADES_SIZE, startingAllocation=ALLOCATION_50_50,
                                        tpValue=maxTP, plottable=False, adaptiveTradesSize=False)
                validBots = []
                pnls = []
                efficencies = []
                params = []

                with ProgressBar(max_value=(maxScore - self.minScore) * (maxMaxOpen - self.minOpen) * (
                        maxClosingAge - self.minAge)) as bar:
                    barValue = 0
                    barFull = False
                        ...
                        for open in range(self.minOpen, maxMaxOpen + 1):
                            for age in range(self.minAge, maxClosingAge + 1):
                                try:
                                    ...
                                    runner.runSimpleStrategyBacktest(activationScore=score, maxOpen=open,
                                                                     closingAge=age)
                ...
                self.strategyParams.extend(params)
                ...
                if self.validOptimization():
                    ...
                    raise RuntimeError("Inconsistent bot data generated!")
            else:
                raise AttributeError("Check input attributes")

        else:
            raise AttributeError("Check the loaded dataset!")

    def __str__(self):
        if len(self.bots) != 0:
            str = {
                "content": f"{self.loadedDataset.dataset.__str__()}\n\nStrategy optimization results:\nTotal tried configuration: {self.validConfigs} out of {self.totalPossibleConfigs} possible --> "
...
class BacktestGraph():
    ...
    def setupPriceData(self):
        self.linePlot.add_trace(
            go.Scatter(x=self.xDates, y=self.dataset.getLoadedCloses(False), name=self.dataset.market + " price"),
            secondary_y=False)
        self.candlePlot.add_trace(go.Candlestick(x=self.xDates, close=self.dataset.getLoadedCloses(False),
                                                 open=self.dataset.getLoadedOpens(False),
                                                 high=self.dataset.getLoadedHighs(False),
                                                 low=self.dataset.getLoadedLows(False),
                                                 name=self.dataset.market + " price"), secondary_y=False)
        self.heikenPlot.add_trace(
            go.Candlestick(x=self.dataset.getLoadedDates(True), close=self.dataset.getLoadedCloses(True),
                           open=self.dataset.getLoadedOpens(True),
                           high=self.dataset.getLoadedHighs(True), low=self.dataset.getLoadedLows(True),
                           name=self.dataset.market + " price"), secondary_y=False)
    ...
    def showPriceDatatWithMA(self, MAperiod1=int, MAperiod2=int):
        if MAperiod1 in BacktestCandle.SMAs_periods and MAperiod2 in BacktestCandle.SMAs_periods:
            self.candlePlot.add_trace(
                go.Scatter(x=self.xDates, y=self.dataset.getLoadedSMA(MAperiod1), name=f"{MAperiod1} SMA",
                           line={"color": "blue"}), secondary_y=False)
            self.candlePlot.add_trace(
                go.Scatter(x=self.xDates, y=self.dataset.getLoadedSMA(MAperiod2), name=f"{MAperiod2} SMA",
    ...
    def showBacktest(self):
        if self.loaded:
            ...
            raise RuntimeError("Backtest data is not loaded on the plot!")

    def loadUSDequivalentsOnGraph(self, withCandles=bool):
        if withCandles:
            self.performancePlot = self.candlePlot
        else:
            self.performancePlot = self.linePlot
        self.performancePlot.add_trace(go.Scatter(x=self.xDates, y=numpy.array(self.performanceData.get("buyNholdUSD")),
                                                  name="Running buy and hold trade USD equivalent",
                                                  line={"color": "orange"}), secondary_y=True)
        self.performancePlot.add_trace(
            go.Scatter(x=self.xDates, y=numpy.array(self.performanceData.get("runningBotUSD")),
                       name="Running bot total USD equivalent",
                       line={"color": "darkgrey"}), secondary_y=True)
        self.performancePlot.update_layout(
            title=f"Perfromance of backtest on {self.dataset.market} with timestamp/ID: {self.backtestID}",
            yaxis_title="Price ($)")
    ...
    def loadPerformanceOnGraph(self, withCandles=bool):
        if withCandles:
            ...
            self.performancePlot = self.linePlot
        self.performancePlot.add_trace(go.Scatter(x=self.xDates, y=numpy.array(self.performanceData.get("buyNholdPNL")),
                                                  name="Running buy and hold trade PNL (%)",
                                                  line={"color": "orange"}), secondary_y=True)
        self.performancePlot.add_trace(
            go.Scatter(x=self.xDates, y=numpy.array(self.performanceData.get("runningPNL")), name="Running PNL (%)",
                       line={"color": "darkgrey"}), secondary_y=True)
        self.performancePlot.add_trace(go.Scatter(x=self.xDates, y=numpy.array(self.performanceData.get("runningEff")),
                                                  name="Running Efficency (%)",
