...
class TestBasicBacktestModules(unittest.TestCase):
    ...
    def testAdditionalDataLoading(self):  # moving averages and indicators
        self.assertListEqual([60930.1887, 60083.682, 51844.5514, 48122.9939, 45288.002, 0.0],
                             self.aDataset.candles[300].getAllSMAs())
        self.assertListEqual([60718.778, 59061.8056, 54143.31, 49882.1375, 47898.2098, 0.0],
                             self.aDataset.candles[300].getAllEMAs())
    ...
    def testBuyTradeBehaviour(self):
        ...
        self.assertEqual(self.aBuyTrade.getAgeInCandles((BacktestDataset.timeframes.get(aTimeframe)) + 1, aTimeframe),
                         1)
    ...
    def testSellTradeBehaviour(self):
        ...
        self.assertEqual(self.aSellTrade.getAgeInCandles((BacktestDataset.timeframes.get(aTimeframe)) + 1, aTimeframe),
                         1)
...
class TestBacktestBot(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.aBot = BacktestBot(self.aDataset, DEFAULT_INITIAL, DEFAULT_TRADES_SIZE, ALLOCATION_50_50, MINIMAL_TP * 10,
                                False, False)
...
class TestBacktestRunning(unittest.TestCase):
    ...
    def setUp(self):
        self.loader = BackTestLoader()
        self.loader.loadYearlyBacktest(aTimeframe, aMarket, aYear, 2)
        self.runner = BackTestRunner(self.loader.dataset, "1D", "BTCUSDT", initialUSD=DEFAULT_INITIAL,
                                     tradesSizeUSD=DEFAULT_TRADES_SIZE,
                                     startingAllocation=ALLOCATION_50_50, tpValue=10, plottable=True,
                                     adaptiveTradesSize=False)
